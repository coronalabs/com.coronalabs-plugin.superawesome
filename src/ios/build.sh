...
OUTPUT_DIR=$1
TARGET_NAME=SuperAwesomePlugin
OUTPUT_SUFFIX=a
CONFIG=Release
...
build_plugin_structure() {
	...
	if ls "$path"/EmbeddedFrameworks/*.a 1> /dev/null 2>&1; then
		echo "Copying static libraries for $PLATFORM:"

		for f in "$path"/EmbeddedFrameworks/*.a; do
			LIBN=$(basename "$f")
			BIN_NAME=${FRAMEWORK_NAME%.framework}
			SRC_BIN="$f"
			DEST_BIN="$PLUGIN_DEST/$LIBN"
			lipo "$SRC_BIN" $ARCH -o "$DEST_BIN.tmp"
			$(xcrun -f bitcode_strip) "$DEST_BIN.tmp" -r -o "$DEST_BIN"
			rm "$DEST_BIN.tmp"
		done
	else
		echo "No 3rd static libraries"
	fi
}
